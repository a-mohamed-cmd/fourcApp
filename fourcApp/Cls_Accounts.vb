...
#End Region
    Public Sub import_Data(
                   Optional code As String = "",
                   Optional genID As Integer = 0,
                           Optional account_gen_name As String = "",
Optional name As String = "",
Optional isActive As Boolean = True)

        _AccountSub_Code = code
        _account_gen.AccountGen_ID = genID
        _account_gen.AccountGen_NameAr = account_gen_name
        _AccountSub_Name = name
        _AccountSub_ISActive = isActive
    End Sub
    ''' <summary>
    ...
    Sub Load_Data(cur As CurrencyManager)
        ...
        AccountSub_ID = cur.Current("AccountSub_ID")
        AccountSub_Code = cur.Current("AccountSub_Code")
        account_gen.AccountGen_ID = If(IsDBNull(cur.Current("AccountGen_ID")),
                                    Nothing, cur.Current("AccountGen_ID"))
        account_gen.AccountGen_NameAr = If(IsDBNull(cur.Current("AccountGen_NameAr")),
                                    Nothing, cur.Current("AccountGen_NameAr"))

        AccountSub_Name = cur.Current("AccountSub_Name")
        AccountSub_ISActive = If(IsDBNull(cur.Current("AccountSub_ISActive")),
                                            Nothing, cur.Current("AccountSub_ISActive"))

    End Sub
    Sub Send_Data(txt() As Guna2TextBox, swt As Guna2ToggleSwitch)
        txt(0).Text = account_gen.AccountGen_ID
        txt(1).Text = account_gen.AccountGen_NameAr
        txt(2).Text = AccountSub_Code
        swt.Checked = AccountSub_ISActive
        txt(3).Text = AccountSub_Name
...
#Region "Methods"
    ...
    Public Function View_StoresAndDepartments() As DataTable
        ...
        ' إعداد الإجراء المخزن
        View_StoresAndDepartments = sqlMethod.ExecuteSelectData("Accounts.viewpaymentsAccounts")
        ' التحقق من النتيجة
