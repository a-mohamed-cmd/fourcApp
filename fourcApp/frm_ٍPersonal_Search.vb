...
Public Class frm_Personal_search

    Private _dt As New DataTable
    Private _frm As Object = frm_PurchaseReceipt
    ...
    Private Sub load_Items_list()
        _dt = _frm.View_SuppliersData()

        dgv_ItemsMove.DataSource = _dt
        If _dt.Rows.Count = Nothing Then Exit Sub
        _frm.Dgv_Design_view_items(dgv_ItemsMove)
    End Sub
    Private Sub clear()
    ...
    Private Sub Search_Mode_items()
        Select Case Com_searchMode.SelectedIndex
            Case 0
                _dt = _frm.Search_by_code(txt_SearchInfo.Text)
            Case 1
                _dt = _frm.Search_by_NameAr(txt_SearchInfo.Text)
        End Select
    End Sub
